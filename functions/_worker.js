export default {
  async fetch(request, env) {
    ...
    // strict allowlists
    const allowedOrigins = ["*.bitsonline.nl", "*.indehoven.com"];
    ...
    const count = parseInt((await env.proxy-worker_events.get(rateKey)) || "0", 10);
    if (count > 300) return response(429, "Rate limit exceeded");
    await env.proxy-worker_events.put(rateKey, (count + 1).toString(), { expirationTtl: 60 });
...
function log(env, origin, target, method, status, cache) {
  try {
    env.proxy-worker_log.writeDataPoint({
